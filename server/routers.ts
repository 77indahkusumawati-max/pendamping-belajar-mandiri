...
import {
  ...
  getAIConversation,
  getBookmarkedSubjects,
  getHiddenMaterialComments,
  getLeaderboard,
  getManagedMaterials,
  getMaterialComments,
  ...
  upsertStudyProgress,
  deleteManagedMaterial,
} from "./db";
import { storageGetSignedUrl, storagePut } from "./storage";
import { invokeLLM } from "./_core/llm";

const extractedStudySchema = z.object({
...
export const appRouter = router({
    ...
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      return {
        ...
        activityDates: saved?.activityDates
          ? JSON.parse(saved.activityDates)
          : [],
  ...
  quiz: router({
    submit: protectedProcedure
      .input(
        z.object({
          quizKey: z.string().max(64),
          score: z.number().int().min(0),
          total: z.number().int().positive(),
        })
      )
      .mutation(({ ctx, input }) =>
        addQuizAttempt({ userId: ctx.user.id, ...input })
      ),
    recent: protectedProcedure.query(({ ctx }) =>
      getRecentQuizAttempts(ctx.user.id)
    ),
  }),
  admin: router({
    ...
    saveMaterial: adminProcedure
      .input(
        z.object({
          ...
          steps: z.string().trim().min(10).max(10000),
          source: z.string().trim().min(2).max(255),
          level: z.enum(["TK", "SD", "SMP", "SMA", "SMK", "Kuliah"]),
          difficulty: z.enum(["Pemula", "Menengah", "Lanjutan"]),
          track: z.string().trim().min(2).max(80),
        })
      )
      .mutation(({ ctx, input }) =>
        upsertManagedMaterial({ ...input, createdBy: ctx.user.id })
      ),
    deleteMaterial: adminProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ input }) => deleteManagedMaterial(input.id)),
    restoreComment: adminProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ input }) => moderateMaterialComment(input.id, "visible")),
    deleteComment: adminProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ input }) => deleteMaterialComment(input.id, 0, true)),
  }),
  preferences: router({
    get: protectedProcedure.query(({ ctx }) =>
      getStudyPreferences(ctx.user.id)
    ),
    save: protectedProcedure
      .input(
        z.object({
          interests: z.array(z.string().min(1).max(80)).max(10),
          preferredTrack: z.string().min(1).max(80),
        })
      )
      .mutation(({ ctx, input }) =>
        upsertStudyPreferences({
  ...
  uploads: router({
    list: protectedProcedure.query(({ ctx }) =>
      getUploadedMaterials(ctx.user.id)
    ),
    create: protectedProcedure
      .input(
        z.object({
          title: z.string().trim().min(2).max(180),
          category: z.string().trim().min(2).max(80).default("Umum"),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (buffer.length > 5_000_000)
          throw new Error("Ukuran materi maksimal 5 MB");
        const stored = await storagePut(
          `user-materials/${ctx.user.id}/${input.fileName}`,
          buffer,
          input.mimeType
        );
        return saveUploadedMaterial({
          userId: ctx.user.id,
          title: input.title,
    ...
    delete: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) =>
        deleteUploadedMaterial(ctx.user.id, input.id)
      ),
    extract: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!upload)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Materi unggahan tidak ditemukan",
          });
        if (upload.mimeType !== "application/pdf")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Ekstraksi AI saat ini hanya mendukung PDF",
          });
        let pdfUrl: string;
        try {
          pdfUrl = await storageGetSignedUrl(upload.fileKey);
        } catch {
          throw new TRPCError({
            code: "PRECONDITION_FAILED",
            message:
              "PDF belum dapat diakses dari penyimpanan. Coba unggah ulang file.",
          });
        }
        let response;
        try {
            ...
            messages: [
              ...
              },
              {
                ...
                content: [
                  {
                    type: "text",
                    text: "Ekstrak materi PDF ini menjadi ringkasan dan kuis interaktif untuk pelajar.",
                  },
                  {
                    type: "file_url",
          ...
          throw new TRPCError({
            ...
            message:
              "Teman AI belum dapat memproses PDF saat ini. Silakan coba lagi nanti.",
          });
        }
        ...
        try {
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "AI belum dapat membaca PDF menjadi ringkasan dan kuis yang valid. Coba unggah PDF yang lebih jelas.",
          });
        }
      }),
  }),
  materials: router({
    ...
    bookmarks: protectedProcedure.query(({ ctx }) =>
      getBookmarkedSubjects(ctx.user.id)
    ),
    toggleBookmark: protectedProcedure
      .input(z.object({ subject: z.string().min(1).max(120) }))
      .mutation(({ ctx, input }) =>
        toggleMaterialBookmark(ctx.user.id, input.subject)
      ),
    ...
    addComment: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        addMaterialComment({ userId: ctx.user.id, ...input })
      ),
    updateComment: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        updateMaterialComment(input.id, ctx.user.id, input.body)
      ),
    deleteComment: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(({ ctx, input }) =>
        deleteMaterialComment(input.id, ctx.user.id, ctx.user.role === "admin")
      ),
    moderateComment: adminProcedure
      ...
      .mutation(({ input }) => moderateMaterialComment(input.id, input.status)),
    conversation: protectedProcedure
      .input(z.object({ subject: z.string().min(1).max(120) }))
      .query(({ ctx, input }) => getAIConversation(ctx.user.id, input.subject)),
    askAI: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
          ...
          messages: [
            {
              role: "system",
              content:
                "Kamu adalah Teman AI, pendamping belajar untuk pelajar SMK. Jawab dalam bahasa Indonesia yang sederhana, bertahap, dan tidak mengarang sumber. Jika pertanyaan di luar materi, arahkan kembali dengan sopan.",
            },
            {
              role: "user",
              content: `Materi: ${input.subject}\nPertanyaan: ${input.question}`,
            },
          ],
        });
        const answer = String(
        ...
        await saveAIConversation(ctx.user.id, input.subject, [
          { role: "user", message: input.question },
